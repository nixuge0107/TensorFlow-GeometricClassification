...
Y = tf.placeholder(tf.int64, shape=[None], name='labels_placeholder')
...
def inference(batch_size, n_classes):
    ...
    with tf.variable_scope('local3') as scope:
        ...
        reshape = tf.reshape(pool2, shape=[-1, shape])
        # dim = reshape.get_shape()
        print("dim", shape)
        weights = tf.Variable(tf.truncated_normal(shape=[shape, 128], stddev=0.005, dtype=tf.float32),
                              name='weights', dtype=tf.float32)
    ...
    with tf.variable_scope('softmax_linear') as scope:
        ...
        # 预测的label
        prediction_labels = tf.argmax(softmax_linear, axis=1, name="output")

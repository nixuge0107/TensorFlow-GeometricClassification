...
def recognize(jpg_path, pb_file_path):
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            print(input_x)
            out_softmax = sess.graph.get_tensor_by_name("softmax_linear/softmax:0")
            print(out_softmax)
            # out_label = sess.graph.get_tensor_by_name("softmax_linear/output:0")
            # print(out_label)

            img_out_softmax = sess.run(out_softmax, feed_dict={input_x: batch_x})
...
recognize("D:/train_data/image_data/input_data/rectangle/1146samples2.jpg",
